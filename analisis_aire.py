...
def ej_1_cargar_datos_demograficos() -> pd.DataFrame:
    # Cargar datos demográficos desde una URL
    ...
    # Realizar limpieza de datos demográficos
    data.drop(['Race', 'Count', 'Number of Veterans'], axis=1, inplace=True)
    data.drop_duplicates(inplace=True)
    
    return data

def ej_2_cargar_calidad_aire(ciudades: set) -> None:
    # Crear tabla de dimensiones para almacenar datos de calidad del aire
    air_quality_data = pd.DataFrame(columns=['city', 'CO', 'NO2', 'O3', 'SO2', 'PM2.5', 'PM10', 'overall_aqi'])
    
    # Obtener datos de calidad del aire para cada ciudad utilizando una API externa
    for city in ciudades:
        api_url = f"https://api-ninjas.com/api/airquality?city={city}"
        response = requests.get(api_url)
        
        # Procesar la respuesta de la API y agregar datos a la tabla de calidad del aire
        if response.status_code == 200:
            air_quality = response.json()
            air_quality_data = air_quality_data.append({
                'city': city,
                'CO': air_quality['data']['concentration']['CO'],
                'NO2': air_quality['data']['concentration']['NO2'],
                'O3': air_quality['data']['concentration']['O3'],
                'SO2': air_quality['data']['concentration']['SO2'],
                'PM2.5': air_quality['data']['concentration']['PM2.5'],
                'PM10': air_quality['data']['concentration']['PM10'],
                'overall_aqi': air_quality['data']['concentration']['overall_aqi']
            }, ignore_index=True)

    # Crear base de datos SQLite y cargar tablas
    ...
    air_quality_data.to_sql('calidad_aire', conn, index=False, if_exists='replace')
    conn.close()
